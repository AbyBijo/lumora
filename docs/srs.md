...
- Passwords bcrypt-hashed (cost 12); password policy ≥8 chars + letter +
  number. Sessions are server-side, revocable, 30-day TTL, sliding; cookie is
  httpOnly + SameSite; only SHA-256 of the token is stored.
- Auth endpoints rate limited (10/15 min/IP); CSRF origin check on all
  mutating API requests; security headers set globally.
...
- **Security** — bcrypt passwords, server-side sessions, CSRF origin check,
  per-IP rate limits, security headers (CSP, X-Frame-Options, nosniff); no
  secrets in the client; provider keys server-side only.

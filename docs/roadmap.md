...
- [x] **Real auth** — bcrypt accounts, server-side revocable sessions,
      rate limiting, CSRF protection (ADR 0008). NextAuth/Clerk remains an
      option via the `src/lib/auth/` seam; email verification is Phase 2.
...
- [ ] **Async processing** — Redis + BullMQ worker for large documents; pipeline
      progress over websockets/SSE (rate limiter is also Redis-ready, see docs/security.md).

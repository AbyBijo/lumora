...
# 3. Create the database + seed the demo workspace
npx prisma db push
npm run db:seed

# 4. Run
npm run dev
# open http://localhost:3000 — sign in as "Demo Learner"
```
...
The seed creates a demo curriculum (“The Science of Learning and Memory”) with
quizzes, flashcards, mastery history, and a shaped study streak, so the product
...
# → open http://localhost:3000 on your phone, sign in as "Demo Learner"
```
...
│   ├── smoke.ts                 # engine smoke test
│   └── screenshots.ts           # browser screenshots (needs `npm i playwright`)
└── src/
    ├── app/                     # Next.js App Router (pages + API routes)
    ├── components/              # UI (ui/, study/, flashcards/, curriculum/, upload/)
    ├── engine/                  # parsing, chunking, curriculum, quizzes, LLM gateway
    ├── lib/                     # db, auth, srs (SM-2), mastery, services
    └── types/
```
